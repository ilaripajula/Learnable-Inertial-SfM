...
def plot_refined_pose(outputs, errors, conf, phase, scan, epoch=None, bundle_adjustment=False):
    Rs_gt = outputs['Rs_gt']
    ts_gt = outputs['ts_gt']

    Rs_pred = outputs['Rs_refined']
    ts_pred = outputs['ts_refined']
...
def plot_cameras(Rs_pred, ts_pred, pts3D, Rs_gt, ts_gt, Rs_error, ts_error, conf, phase, scan=None, epoch=None):
    ...
    fig.update_layout(
        title='Cameras: Rotation Mean = {:.5f}, Translation Mean = {:.5f}'.format(Rs_error.mean(), ts_error.mean()),
        showlegend=True,
        scene=dict(
            aspectmode='manual',  # Ensure we are setting the aspect ratio manually
            aspectratio=dict(x=1, y=1, z=1)  # Set the aspect ratio for x, y, z axes to be equal
        )
    )

    path = utils.path_utils.path_to_plots(conf, phase, epoch=epoch, scan=scan) + 'TRAJ'
