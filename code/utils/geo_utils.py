...
def align_cameras_without_scale(pred_Rs, gt_Rs, pred_ts, gt_ts, return_alignment=False):
    '''
    Aligns predicted camera poses (rotations and translations) with ground truth poses without scale factor.
    ...
    :param return_alignment: bool, whether to return the similarity transformation matrix
    :return: Aligned rotations and translations (and optionally the similarity matrix)
    '''
    ...
    # Create sv to ensure proper rotation matrix
    sv = np.ones(3)
    ...
    # Apply optimal rotation to predicted rotations
    R_fixed = R_opt.reshape([1, 3, 3]) @ pred_Rs
    
    # Find optimal translation
    pred_ts_rotated = pred_ts @ R_opt.T  # Apply the optimal rotation on all the translations
    
    # Define optimization variable for translation
    t_opt = cp.Variable((1, d))
    
    # Define objective and constraints
    constraints = []
    obj = cp.Minimize(
        cp.sum(cp.norm(gt_ts - (pred_ts_rotated + np.ones((n, 1), dtype=np.double) @ t_opt), axis=1)))
    ...
    # Compute aligned translations
    t_fixed = pred_ts_rotated + t_opt.value.reshape([1, 3])
    
    if return_alignment:
        # Construct similarity transformation matrix without scale
        similarity_mat = np.eye(4)
        similarity_mat[0:3, 0:3] = R_opt
