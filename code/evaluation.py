...
def prepare_predictions(data, pred_cam, conf, bundle_adjustment):
    ...
    if calibrated:
        ...
        if 'IMU_refined_poses' in pred_cam.keys():
            Ps_refined = pred_cam["IMU_refined_poses"].cpu().numpy()  # Normalized camera!!
            #Ps_refined = Ns_inv @ Ps_refined  # unnormalized cameras
            Rs_pred_refined, ts_pred_refined = geo_utils.decompose_camera_matrix(Ps_refined)
            outputs['Rs_refined'] = Rs_pred_refined
            outputs['ts_refined'] = ts_pred_refined
        ...
        outputs['Rs_fixed'] = Rs_pred #Rs_fixed
        outputs['ts_fixed'] = ts_pred #ts_fixed
        outputs['pts3D_pred_fixed'] = pts3D_pred #(similarity_mat @ pts3D_pred)  # 4,n
        outputs['pts3D_triangulated_fixed'] = pts3D_triangulated #(similarity_mat @ pts3D_triangulated)
