...
class SceneData:
    def __init__(self, M, Ns, Ps_gt, scan_name, dilute_M=False, imu_list=None):
        ...
        # Prepare Ns inverse (not used anywhere)
        ...
        # Load pickle file with IMU.
        if imu_list is None:
            with open(f'/home/ilari/Documents/Equivariant-SFM/datasets/Euclidean/IMU/{scan_name}.pkl', 'rb') as file:
                self.imu_list = pickle.load(file)
            self.imu_list = [torch.tensor(imu).float() for imu in self.imu_list]
        else:
    ...
    def to(self, *args, **kwargs):
        for key in self.__dict__:
            if not key.startswith('__'):
                ...
                #         setattr(self, key, attr[i].to(*args, **kwargs))
...
def get_subset(data, subset_size):
    ...
    M = data.M[M_indices]
    M = M[:, (M > 0).sum(dim=0) > 2]
    imu_list = [data.imu_list[i] for i in indices[:-1]]
    return SceneData(M, Ns, y, data.scan_name + "_{}".format(subset_size), imu_list=imu_list)
